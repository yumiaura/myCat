...
### Added
- The **long break (15–30 min after every 4 pomodoros) is now marked "Big break"** — a "🛋 Big break" row in the table, "▶ Big break" on the live current row, and "Big break" in the countdown and the flyby banner — so it reads distinctly from the short 5-minute rest (branch `feat/companion`).
- **Activity timeline in the Activity dialog.** A per-minute heat strip of the day: grey where nothing was tracked, green where you were at rest, and red — deeper the busier — where you were active, so the work→rest→next-period rhythm is visible at a glance (hover for the time). The strip always spans the **full day** (midnight→midnight); past-untracked minutes are a medium grey, the still-to-come future is a lighter grey, and a prominent blue "now" marker (a line with a triangle) shows how far along we are. The live **Current row is labelled by phase** — ▶ Focus / ▶ Rest / ▶ Big break during a pomodoro, or ▶ Active (no timer) when you're active with no timer running (dropping the earlier Work/Working overlap). The dialog is larger and the checkboxes now read "Track my activity" / "…count keys + clicks too" with the detail in tooltips (branch `feat/companion`).
