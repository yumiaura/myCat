...
- **A dev tool that bakes a VRM into a myCat character.** New `tools/bake_vrm.py` poses a VRM/glb (via `mycat/vrm_pose.py`), renders it headless with QtQuick3D onto a transparent canvas, autocrops, and packs it as a standard `<id>.zip` (`config.json` + `static.png`). The baked zip drops straight into the chars dir and shows up in the right-click **Chars** menu with **no runtime code change** — the app loads it like any other character. It is a build-time tool only (not imported by the app, no new runtime dependency); run it with `xvfb-run` on a headless box. VRM MToon materials fall back to PBR, which reads fine for a still (branch `feat/vrm-pose-math`).
