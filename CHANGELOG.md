...
### Changed
- The Activity dialog now shows a **per-session table** — one row per focus/break session with its start time, duration, keystrokes, clicks and cursor path — and a bold **totals row at the bottom** with the day's aggregates (no start/end times), plus the live "Now" line at the top mirroring the focus tooltip, refreshed every second while the dialog is open (branch `feat/companion`).
