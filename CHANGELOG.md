...
### Changed
- The Activity dialog shows a **per-session table**: a live **"Current" row on top** (in-progress period — start time, "Current" in the Duration cell, keys/clicks/path updating every second), then finished sessions **newest-first**, then a bold **TOTAL row** with the pomodoro count and day aggregates (total duration with no "active" wording, no start/end times). Labels no longer elide to "Focus …". The "Now" line above mirrors the focus tooltip (branch `feat/companion`).
