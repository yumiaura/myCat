...
### Changed
- The Activity dialog opens with a live **"Now" line** mirroring the focus tooltip — countdown plus the session's interim stats (⌨ keys, 🖱 path, % active), refreshed every second while the dialog is open (branch `feat/companion`).
