...
### Added
- The **long break (15–30 min after every 4 pomodoros) is now marked "Big break"** everywhere — a deeper-green block in the timeline, a "🛋 Big break" row in the table, "▶ Big break" on the live current row, and "Big break" in the countdown and the flyby banner — so it reads distinctly from the short 5-minute rest (branch `feat/companion`).
- **Day-rhythm timeline in the Activity dialog.** A horizontal strip visualises the day's pomodoro rhythm — red focus blocks, green rest, amber interrupted, a blue live "current" lane, hour axis and a "now" marker — so the work→rest→next-period cadence is visible at a glance (hover a block for its type and time span). The live **Current row now says plainly Work / Rest / Working** (colour-coded) instead of an ambiguous "Current", so it's obvious whether you should be working or resting (branch `feat/companion`).
