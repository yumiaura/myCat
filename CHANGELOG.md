...
- **Activity timeline in the Activity dialog.** A per-minute heat strip of the day: grey where nothing was tracked, green where you were at rest, and red — deeper the busier — where you were active, so the work→rest→next-period rhythm is visible at a glance (hover for the time). The strip spans the **full selected day** (midnight→now for today, the whole 24 h for yesterday). The live **Current row says plainly Work / Rest / Working** (colour-coded) so it's obvious whether to work or rest. The dialog is larger and the checkboxes now read "Track my activity" / "…count keys + clicks too" with the detail in tooltips (branch `feat/companion`).
