...
The character can be generated by OpenAI (hosted) or by a local Stable Diffusion
(AUTOMATIC1111) or ComfyUI server. Each backend offers txt2img (prompt only) and
img2img (uses the reference photos), and the local ones can list their
checkpoints so you can pick a model. All settings persist in config.ini.
"""
...
BACKEND_LABELS = [
    ...
    ("Stable Diffusion — local server", "a1111"),
    ("ComfyUI — local server", "comfyui"),
]
MODE_LABELS = [
...
class AICharDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.local_group = QtWidgets.QGroupBox("Local server")
    ...
    def generate(self) -> None:
        ...
        if kind in ("a1111", "comfyui") and not self.url_edit.text().strip():
            self.set_status("Enter the local server address.", error=True)
            return
        try:
