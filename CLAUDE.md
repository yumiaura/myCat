...
The line above imports the maintainer's private working notes. That file is
gitignored, so in a fresh clone it simply does not exist and the import resolves
to nothing. Everything below is the public part.
...
`tests/` holds over 30 pytest files and CI (`.github/workflows/ci.yml`, Python 3.10
and 3.12) runs ruff plus the pytest line above. Two rules make the suite survivable:
...
- `llm*.py`: chat. `llm_vendors.py` is the vendor registry (built-in presets plus
  ...
  both written on stdlib `urllib`, so there is no third-party API client to install.
...
- `shop_api.py` / `shop_ui.py`: the char shop client. **Unfinished and switched
  off**, the menu entry is commented out in `main.py`, so nothing calls it. Do not
